...
if calculate_btn:
    ...
    for month in range(1, loan_term + 1):
        ...
        row = {
            ...
            "процентная часть": round(interest_pay, 2),
            "долговая часть": round(principal_pay, 2),
    ...
    col1.metric("Ежемесячный / Первый (аннуитет / дифференцированный) платёж", f"{df['Ежемесячный платёж'].iloc[0]:,.2f} ₽")
    ...
    with st.expander("График платежей", expanded=True):
        st.dataframe(
            ...
            hide_index=True,
            column_config={
                "Ежемесячный платёж": st.column_config.NumberColumn(format="%.2f ₽"),
                "процентная часть": st.column_config.NumberColumn(format="%.2f ₽"),
                "долговая часть": st.column_config.NumberColumn(format="%.2f ₽"),
